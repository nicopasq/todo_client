...
function Task ({task, toggleUpdateTask, removeFromTaskList}){
  ...
  function deleteTask(id){
    ...
    <li key={task.id} className="task">
      ...
      >
        <Grid onClick={e => toggleUpdateTask(task.id)}  xs={4}>
          <Typography color="warning" className="taskData" level="h2">
            {task.description}
          </Typography>
        </Grid>
        <Grid xs={4}>
          <Typography onClick={e => toggleUpdateTask(task.id)} sx={{borderRight:"1px solid grey", height:"100%"}} color="info" className="taskData" level="h2">
            {task.category}
          </Typography>
        </Grid>
        <Grid xs={2}>
